...
### 第 1 期：翻译层（spec-v2.md 已完成 2026-04-15，待编码）
- [x] spec-v2.md 技术规格文档
- [ ] 任务 5.1：types.py — 统一数据结构定义
- [ ] 任务 5.2：geo_math.py — 通用地理计算工具
- [ ] 任务 5.3：stats_calculator.py — 通用统计计算
- [ ] 任务 5.4：GPX 解析器重构 — 迁移到 parsing/，输出 ParseResult
- [ ] 任务 5.5：coord_normalizer.py — 坐标系检测 + GCJ-02→WGS84
- [ ] 任务 5.6：Trackpoint 表 Alembic 迁移 — 新增 speed, distance 列
- [ ] 任务 5.7：Worker 改造 — 调用新的 parsing 模块
- [ ] 任务 5.8：FIT 解析器 — fit_parser.py
- [ ] 任务 5.9：上传接口改造 — 支持 .fit 文件
- [ ] 任务 5.10：翻译层测试

### 四步战略规划
- [x] 第 0 期：地基修补 ✅
- [ ] 第 1 期：翻译层 — spec 已完成，待编码
- [ ] 第 2 期：Strava 集成 — OAuth + Webhook + 渐进式历史导入
