...
| 风险 | 级别 | 说明 | 当前应对 | 修复状态 |
|------|------|------|---------|---------|
| Worker 僵尸 | 🔴 | activity 卡在 processing/pending | 轮询接口超时 10 分钟标记 failed（仅 processing） | ⚠️ pending 僵尸无人管 |
| 重复上传 | 🔴 | 用户双击 → 两条重复 activity | 前端防抖，后端零防护 | ❌ 未实现 |
| 内存爆炸 | 🔴 | 50MB GPX → 50 万轨迹点 → 峰值 400MB | 文件大小限制 50MB，轨迹点数无上限 | ❌ 未实现 |
| 连接池不足 | 🔴 | pool_size=5 默认值 | 无 | ❌ 未配置 |
| Worker 重入 | 🔴 | RQ 超时重试 → 双重处理同一任务 | 无 | ❌ 未防护 |
