...
### 第 2 期：Strava 集成（spec 已完成，待审查后编码）
- [ ] 任务 6.1：Strava OAuth 接入 — 用户授权流程
- [ ] 任务 6.2：Strava API 客户端 — 封装请求 + 限流
- [ ] 任务 6.3：Strava 适配器 — from_streams() 实现
- [ ] 任务 6.4：导入调度器 — 三层渐进策略 + 轮转 + 断点续传
- [ ] 任务 6.5：Webhook 接收 — 新活动自动导入
- [ ] 任务 6.6：导入进度 API — 前端显示"导入中 50%"
- [ ] 任务 6.7：Strava 集成测试
