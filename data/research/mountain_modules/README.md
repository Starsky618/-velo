...
6. 一个或多个 MountainModule、TransitPath 与来源走廊形成真实骑法后，写版本化 choice spec 并交给通用组合入口；区域只提供数据，不复制组合程序：
   ...
   结果必须分别说明骑法范围、距离、GLO 爬升下降、同方向热度范围、未观测比例、适合谁、为什么与代价。没有风景、路面、车流、补给硬事实时不脑补。该入口负责组装、hard reject 和硬事实对比；只有同一 rider job 的候选才另调用 `rank_heat_candidates()` 做 Pareto/意图排序。程序 replay loop 已可用，产品 learning loop 仍待真实曝光、选择/拒绝、完成/放弃 episode。
...
区域批处理先用 `scripts/analyze_regional_route_cognition.py` 对固定窗口的 exact observation set 做身份、几何 hash、GLO 总量、oracle subset 与 family projection 对账。自然相邻端口可用 `scripts/build_transit_provider_snapshot.py` 生成腾讯骑行或开车 connectivity shadow，再用 `scripts/analyze_xishan_transit_path.py` 重放；`--module-manifest` 可重复传入多个模块。多个模块与 TransitPath 的选择集统一交给 `scripts/analyze_route_choice_set.py`，`--module-run` 和 `--transit-run` 均可重复传入。
