...
- `mode = snap` 只允许 `gcj02`，因为小程序地图手势点来自国内地图展示坐标，且腾讯贴路使用 GCJ-02。
- `mode = freehand` 同时允许 `gcj02` 和 `wgs84`；后者给已有 GPX、Strava 来源轨迹或内部手绘补丁复用同一条“保留原线、不调腾讯”校验链。
- `mode = snap` 时调用腾讯骑行路线规划。
- `mode = freehand` 时不调用腾讯，只做点数、距离、范围检查后返回原线。
- Strava 赛段投影到本地几何时固定按 `腾讯路径规划 -> OSM 路由 -> Route Draw freehand` 尝试；手绘候选必须记录前两级失败证据，不能跳过路由阶段，也不能把路由绕行冒充原赛段。
- Strava 原始轨迹超过 120 点时，自动手绘工具在最大横向误差 3m 内寻找满足点数上限的最小 RDP 阈值；若 3m 内仍超过 120 点，则分段或交由人工描绘，不继续加大误差。
- 自动手绘必须检查原始轨迹连续性；来源相邻点超过 manifest 允许间距时拒绝生成，不能用一条隐式直线跨过缺失区间。
- 输入点少于 2 个拒绝。
- 单次原始触摸点最多 120 个。
