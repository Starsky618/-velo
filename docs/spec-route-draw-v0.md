...
骑友在“探索”页点“画一条路线”，用手指沿着自己想骑的路描线；VELO 在用户松手后把这一小段尽量贴到可骑行道路上，用户确认后保存成自己的路书。保存后的路线走 VELO 自己的路线版本、统一海拔、GPX/TCX 导出，不把腾讯地图当成长期数据主人。

## 用户故事

周五晚上，小明想规划周末的西山绕圈。他不想只选起点终点，因为自动规划总把他带到不想骑的主路。于是他打开 VELO 的“探索”页，点“画一条路线”，在地图上沿着熟悉的路一段一段描。

每画完一段，VELO 给他一条橙色贴路结果，灰线保留他刚才手画的大致意图。如果橙线跳到隔壁辅路，他点撤回，重画这一小段；如果这段确实不是路网里的路，他切到“自由画线”。最后他点保存，进入“我的路书详情”：马上看到距离、海拔起伏、总爬升，也能一键去发约骑或导出到码表。
...
- 新增全屏 `route-draw` 页面。
- 用户手指画一段线，松手后再吸附；V0 不做边画边实时吸附。
- 每段都能撤回。
- 支持“贴路模式”和“自由画线模式”。
- 保存后生成 `RouteBook + RouteVersion`，并复用统一 SRTM 海拔链路。
...
实时吸附看起来高级，但用户会感觉系统在抢方向盘。V0 用“画完一段 -> 显示灰色手画线和橙色吸附线 -> 用户确认或撤回”的节奏，能让错误马上暴露。
...
探索页
  -> route-draw 页面采集屏幕点
  -> MapContext.fromScreenLocation 转 GCJ-02 经纬度
  -> 前端按距离/抽稀得到一段原始手画线
...
手势实现要求：
- 先做一个最小手势 spike：用户拖动一段，页面能拿到连续屏幕点，并用 `MapContext.fromScreenLocation` 转成 GCJ-02 经纬度。
- 画线模式开启时禁用地图拖动，避免用户想画线却把地图拖走。
- 可以用透明触摸层或 canvas 层采集手势，但必须真机 / 微信开发者工具验证，不允许只靠静态代码推断。
- 如果 spike 证明原生 map 上无法可靠采集连续画线，不继续做“假画线页”；先回到设计层重新选方案。
...
| 腾讯未配置 key | “路线贴路服务暂时不可用，可以先用自由画线保存。” | 返回 503，前端保留自由画线。 |
| 腾讯超时 | “这段没有贴上路，换短一点再试。” | 不创建路书。 |
| 腾讯返回空路线 | “这段附近没有找到可骑行道路。” | 用户可撤回或切自由画线。 |
...
| 风险 | 严重度 | 对策 |
| --- | --- | --- |
| 吸附到辅路、高架、河对岸 | 高 | 同屏显示灰色原线和橙色吸附线；每段可撤回。 |
