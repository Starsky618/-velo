...
4. 我有没有给 Tim 任何"未来承诺"句式（"记住了 / 学到了 / 待会做 X"）？
   有 → **立刻** save memory / TaskCreate / 写文件落实，不只口头答应。
...
| 问 | 翻车实证 | 教训 |
|---|---|---|
| 1 翻译层 | task-0.7 部署直接贴 PAT / docker config raw 输出到对话 | 信息整流不是 nice-to-have |
| 2 实证差距 | task-0.7 mock 测试 5/5 全过，生产首跑 24/24 炸（reference_line EWKB hex）| mock ≠ 真环境，必跑命令门禁 |
...
- **Claude 看**：接任务前自查 / 该派 Codex 还是自做 / 该让 Tim 主对话
- **Codex 看**：接任务前自查 / **命中反指标有权 push back / 不盲接**（双主驾对称权利）

---

**角色画像**（拟人化 / 一眼秒懂）

- **Codex = "任务执行器"**：清楚边界 / 验收标准 / 可运行命令 → 它能跑得快
- **Claude = "结对同事"**：问题还没说清 / 它愿意先理解上下文 / 追问意图 / 解释取舍

**核心原则**：CEO 不当 CTO 也不当 VP——Tim 决定**谁干什么** / 不纠结"谁是主开发"。每次任务按类型分派 / 让每个 AI 干它强项。

---

**决策三轴**（判断层抽象 / Tim 脑子里装的）

任何任务先过 3 个属性：

| 轴 | 偏 Codex（执行器） | 偏 Claude（结对同事） |
|---|---|---|
| **歧义度** | 低 / spec 清晰 / 工单可写 | 高 / 需求模糊 / 还在权衡 |
| **可测性** | 有明确验收命令（pytest / curl / grep）| 难自动验收 / 靠人类判断 |
| **可回滚** | 改坏能回退 / 影响范围小 | 改坏不可逆 / 跨模块连锁 |

**经验法则**：3 轴中 ≥ 2 轴偏 Codex → 派 Codex / ≥ 2 轴偏 Claude → Claude 干。
...
**Codex 反指标**（命中即 push back / 不盲接 / 双主驾对称权利）
...
**派 Codex 时的 issue 5 字段速查**（Claude 帮 Tim 把 1 句话翻译成 5 字段 issue）

```
1. 背景         上下文 / 当前状态 / 为什么做
2. 目标         要做到什么样 / 验收标准
3. 验收命令     怎么证明做对了（pytest / curl / grep file:line）
4. 不要碰       哪些文件 / 哪些模块 / 哪些规则不能动
5. 失败处理     报错时停下还是继续 / 重试几次
```

**典型误区**：只说"帮我优化一下"。**好的说法**：优化哪条路径 / 目标指标是什么 / 不能牺牲什么 / 如何证明改完没坏。
...
**Trigger 条件**（任一命中即触发）：
- 会话上下文累积量大（长讨论 / 多轮决议 / 跨 session 待办）
- Tim 即将换终端窗口
- spec 链路 / 大 task / 战略讨论收尾
- agent 内部识别"再聊下去会丢上下文"

**动作清单**（按顺序执行）：
1. **写 changelog 章节**（持久化今日重点）：
   - "今天做了什么"（commit hash + 简述）
   - "明日计划"（具体下一步）
   - "待 Tim 触发"项目（如学习意愿 / 专题讨论意愿）
2. **检查 memory 是否需新增**（按 §9 升级路由表查表）
3. **给 Tim "新 session 开场 prompt 模板"**（让他下次复制粘贴）

**新 session 开场 prompt 标准格式**：
```
继续 velo 项目。开工前必读：
1. docs/changelog.md "<最新日期> <主题>"+"待办"章节
2. CLAUDE.md 顶部"🧭 决策反向索引"+"🤝 协作硬规则"
3. <当下要做的具体 task 卡 / spec 章节>
...
**实证（2026-04-29）**：本次 task-0.7 完工后 4 议题战略讨论（3 小时 / 13 commit）—— Tim 提"换窗口怎么续上"——立即触发本机制 → commit `daafe62` 加 changelog "战略升级 + 待办" 章节 + 给 Tim 开场 prompt。**首次显式识别**该工作流（之前每次会话结束都该做但没做）。
...
### ⭐ 规则成熟度原则 + 80% 高频例外（2026-04-29 v2.0 / Tim 双重 push back）

**目的**：防止"一次发现就刻规则"导致熵增失控（Tim 警觉的第三阶问题）。

**默认规则成熟度阶梯**：

| 实证次数 | 该做什么 |
|---|---|
| 第 1 次发现 | save memory（实证沉淀，不立规则）|
| 第 2-3 次 | 进 changelog / 立 experiment 观察 |
| 第 N 次（≥3）| 立规则进文档（`agent-collaboration.md` 或对应位置）|

**⭐ 80% 高频例外**（Tim 2026-04-29 push back 实证）：

> 若现象属于 **80% 工作场景常见任务**（如"工作交接"、"让 AI 决策"等元工作流）—— **第 1 次显式识别即立规则**，不走"≥3 次实证"阶梯。

**理由**：高频场景的"实证次数已经 ≥3"——只是之前没有显式识别。今天才识别 ≠ 今天才发生。

**判断"是否高频"的硬 trigger**：
- 几乎每次 session 都会涉及？ → 高频例外
- 一周之内会发生 ≥3 次？ → 高频例外
- 跨任务 / 跨 sprint / 跨期都会发生？ → 高频例外

只满足 1 条即触发例外。

**实证（2026-04-29）**：
- "工作交接桥梁"（§10.X）—— 80% 高频，立规则
- "让 AI 做决策时找规则依据"（CLAUDE.md 决策反向索引）—— 80% 高频，立规则
- "ssh 命令脱敏 secret"（memory）—— 中频，先 save memory，第 2-3 次实证后再决定升级

**反例（不该立的）**：
- 单次踩坑的具体技术陷阱 → save 进 CLAUDE.md 技术栈陷阱清单（不是新立规则，是补 list）
- 一次性部署变更 → 进 changelog
- 用户偏好（Tim 喜欢压缩输出）→ memory user 类
