...
> 当前状态：Phase A `in_progress`；A0/A0C/A1.1/A1.2/A1.3 已由 Orchestrator `PASS / completed`，A1 parent 保持 `in_progress`。A1.4 为 `ready_to_specify`、尚未开始且无执行授权；A1.5 与 A2–A5 继续 `blocked`。Accepted ADR-015 只完成状态与记忆边界裁决，不授权 Session、Run、Memory 或 Runtime 实现。
...
## 2. 当前 Phase A、已完成 A0/A0C/A1.1/A1.2/A1.3 与待规格化的 A1.4
...
- A1.3 未创建 Session/Run/Memory/Context Compiler、schema、向量库或 Runtime；inferred memory 保持 `DEFERRED`。A1.4 仅为 `ready_to_specify`，尚未开始、无执行授权，必须等待新 Task Packet；A1.5 与 A2–A5 继续 `blocked`。
...
A1.3-M Task Packet 的一次性 squash merge 授权不构成长期权限；A1.3 收口后 git write、merge 与 deploy 权限仍为 false，不授权开始 A1.4。`main` 继续只作为远程权威基线与 PR 目标。
