...
> 当前状态：Phase A `in_progress`；A0/A0C/A1.1/A1.2/A1.3 已由 Orchestrator `PASS / completed`，A1 parent 保持 `in_progress`。A1.4 为 `in_review`，Proposed ADR-016 等待 Orchestrator 审查；A1.5 与 A2–A5 继续 `blocked`。本轮只形成能力、批准与副作用边界，不授权 Runtime 或生产行为。
...
## 2. 当前 Phase A、已完成 A0/A0C/A1.1/A1.2/A1.3 与审查中的 A1.4
...
- A1.4 已由新的 Task Packet 选中，Proposed [ADR-016](../adr/016-为什么在线Agent的能力审批与副作用必须显式化.md) 编码 deny-by-default Capability、七类 effect scope、五类 approval mode、exact grant、幂等/ledger/replay 与骑友贡献 proposal 边界，当前为 `in_review`，不得自判 PASS。
...
- A1.4 未实现 Capability Engine、Approval UI、Side-effect Ledger、Contribution、schema 或 Runtime，也未改变 Provider/export 行为；A1.5 与 A2–A5 继续 `blocked`。
...
当前没有以下授权：生产 Agent、Session/Run/Memory/Context Compiler、Capability Engine、Approval UI、Side-effect Ledger、Contribution、LangGraph、multi-agent、长期推断 Memory、向量数据库、完整 road graph、真实腾讯/DEM、真实导出、公开发布、生产流量、部署或 A1.5/A2–A5 实现。Proposed ADR-016 不是 schema、migration、API、UI、Runtime 或生产副作用授权；任何实现必须由 Orchestrator 另发单一 Task Packet，并继续遵守 Agent 不直连 ORM/SQL/原始 Provider、公共发布和未经确认真实导出的硬边界。

A1.4 Task Packet 只授权专用分支的五文件文档交付与 Draft PR，不授权 merge、deploy 或开始 A1.5。交付后 git write、merge 与 deploy 权限恢复为 false；`main` 继续只作为远程权威基线与 PR 目标。
