...
> 当前状态：Phase A `in_progress`；A0/A0C/A1.1/A1.2 已由 Orchestrator `PASS / completed`，A1 parent 保持 `in_progress`。A1.3 已形成 Proposed ADR-015，状态为 `in_review`；A1.4–A1.5 与 A2–A5 继续 `blocked`。这只是状态与记忆边界的文档裁决候选，不授权 Session、Run、Memory 或 Runtime 实现。
...
## 2. 当前 Phase A、已完成 A0/A0C/A1.1/A1.2 与审查中的 A1.3
...
- A1.3 已由 [ADR-015](../adr/015-为什么世界事实会话运行与长期记忆必须分离.md) 编码为 Proposed 候选：World Fact、User State、Planning Session、Agent Run、Explicit Memory v0、Trace/Eval 分别拥有唯一 Owner，Context 只是一轮模型调用的编译投影；状态为 `in_review`，等待 Orchestrator 决定。
- A1.3 不创建 Session/Run/Memory/Context Compiler、schema、向量库或 Runtime；inferred memory 保持 `DEFERRED`。A1.4–A1.5 与 A2–A5 继续 `blocked`。
...
当前没有以下授权：生产 Agent、Session/Run/Memory/Context Compiler 实现、LangGraph、multi-agent、长期推断 Memory、向量数据库、完整 road graph、真实腾讯/DEM、真实导出、公开发布、生产流量、部署或 A1.4–A1.5 实现。未来设计对象不等于 schema、migration、API 或 runtime 授权；任何实现必须由 Orchestrator 另发单一 Task Packet，并继续遵守 Agent 不直连 ORM/SQL/原始 Provider/公共发布/真实导出的硬边界。

A1.3 Task Packet 只授权在专用分支交付 Proposed ADR-015 与四处状态/索引更新；Draft PR 不得标 Ready、合并或部署。交付后 git write、merge 与 deploy 权限恢复为 false；`main` 继续只作为远程权威基线与 PR 目标。
