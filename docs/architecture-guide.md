...
- 骑前静态路线制作/预览/导出基础：RouteBook/RouteVersion + 腾讯 bicycling/贴路 + GLO-30 + GPX/TCX

### 1.2 不包含(明确排除)

- iOS / Android 原生 app(v7+ 才考虑)
- 骑中实时导航、语音播报和偏航动态重规划(交给外部地图/码表,见 ADR-010 / ADR-013)
- 自建全国道路路由引擎(见 ADR-013)
- LLM 生成坐标或路线几何(见 ADR-013)
