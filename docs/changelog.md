...
**三处落地**（全在 meetup-create，零后端改动）：
1. **报名门槛**：纯 textarea → 预设多选 chips（ELIGIBILITY_TAGS 7 档：无特殊门槛/能跟住均速/会修补胎换胎/有 30km 经验/自备补给/有码表功率计/骑公路车）+ 兜底手打补充。选中标签用 " · " 拼成一句存进现有 str 字段（零迁移）；selected 态由"标签是否在当前文本里"反推，所以手打段与 chips 共存互不吞；草稿恢复/手打/点选三入口都同步选中态。
2. **补给点**：保持手填（地点高度自由）+ 加"最近用过"快选 chips。**纯本地缓存**（wx.getStorageSync 最近 3 个，发布成功才写入）——按防火墙+防过度工程原则，不为次要可选字段建表/端点，真有需求再升级服务端。
3. **安全提示**：已有 chips+textarea，本轮只统一视觉（同 .pv-template-chip）+ 补 maxlength。

**顺手清理**：updateField 删 power/speed 残留分支（已改 picker）；门槛/安全提示 textarea 补 maxlength（微信 textarea 默认 140 会静默截断，显式 100/200 对齐后端 max_length 同时修了这个隐患）。

**集成审 2 Important 已修**：补给点 chip 缺 white-space:nowrap 致长名折行；门槛 textarea 缺前端 maxlength 致拼接+手打可超后端 100 字限。1073 passed。
