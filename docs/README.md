...
| 约骑 | 后端和小程序创建、加入、私圈、照片、战报链路已存在 | 有多轮历史真机走查 | 不把历史部署记录当成当前在线证明 |
...
| 改约骑 | `app/meetup/` + `miniprogram/pages/meetup-*` → architecture/data-flow 对应章节 → 真机用户路径 |
