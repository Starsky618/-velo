...
约骑是"提前约好时间、地点、配速，然后一起出发骑车"。链路覆盖：创建草稿（**3 步流（2026-06 重排）：选路线 → 图二就地编辑全部字段(时间/集合/人数/节奏/补给/说明/照片) → 图一总览确认(适合谁/可见范围/门槛/安全) → 发布**；草稿懒建——加照片/进确认时才落库，需先填集合点）→ 开放报名 → 用户加入/退出 → 出发前截止 → 骑完自动完成。**invite_only 私圈约骑**：详情/join/participants/media 都要带 `?token=<share_token>` 口令（creator/已加入者豁免），否则 404；分享走 `wx.shareAppMessage` 原生转发把带 token 的链接发给微信好友。
