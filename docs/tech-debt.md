...
1. 准备把任一 research-shadow 走廊投影结果用于正式关系、热度排序或路线规划（新增第 2 个仅作算法 holdout 的山区积木不触发）；
...
**2026-08-01 分层瓦片实现基线（已上线，数字仍是上线前实验）**：真实原始轨迹计算出的 z11–z18 稀疏清单为 43,594 块，其中 z11–z15 为 3,351 块，z16–z18 为 40,243 块。逐块把全部最终 PNG 灌进 Redis 的一次本地尝试只生成 920/43,594 就被主动停止；该结果已经足够否定“每次 generation 后全量重画最终 PNG 并塞 Redis”。正式实现采用 RQ 预热原始高精度 source + 真实轨迹覆盖清单；持久层先生成 z3–z15 基础瓦片，z16–z18 在细节请求期间保留 z15 父级红线，真实子瓦片到达后覆盖并持久化。390×780 浏览器曾做 12 次快速跨区/缩放，帧间隔 P50 16.7ms、P95 17.6ms、`>34ms` 为 0；这些数字仍不是微信真机或生产网络指标，不能用来替代上线后的观测日志。
...
**检查入口（2026-08-01 已落地）**：API 日志中的 `heatmap_tile_observation` 以 `key=value` 输出 audience、year、zoom、cache status、source、参与本次构建的 source point 数、输出字节数和总耗时；不记录瓦片 x/y、轨迹坐标、cookie、ticket 或 token。磁盘/Redis 命中时 `source_point_count=not_queried`，避免把“未查询”误写成空轨迹。生产检查用 `sudo docker compose logs api --since 24h | grep 'heatmap_tile_observation'`；Redis 用 `INFO memory`/`INFO stats` 看内存与 eviction；PostgreSQL 用按用户聚合的 activity/Trackpoint 计数查询；服务器用 `docker system df -v` 与 heatmap volume 挂载点磁盘监控观察 PNG 容量。当前是可查询日志，不是自动告警；达到任一触发条件时仍需人工重评。
...
## 🟡 P2 `[SCALE-TRIGGER]`：ClimbPlan v1 先随 RouteVersion 保存可重建 JSON，暂不拆爬坡检索表（2026-08-14）

**当前选择**：每次生成可信完整海拔时，在全分辨率 20m 距离网格上运行 `velo_climb_plan_v1`，把有向爬坡组成、Cat/HC 候选、坡型、500m/1km 持续坡度和最多 60 点的单坡 profile 保存到 `RouteVersion.navigation_metadata_json.climb_plan`。详情只读取同版本且通过当前闭集 schema 的已存计划；旧版本没有已存计划、算法版本不匹配或 JSON 损坏时一律显示 pending，不拿稀疏 `elevation_points_snapshot` 或页面 100 点缩略图同步重放。需要补历史版本时只能走授权离线 backfill，并保存完整输入/结果哈希。该结构让路线版本、海拔来源和算法结果原子绑定，当前无需新表、迁移或双写；它不支持跨路线按 Cat/坡型检索，也不冒充设备端实时导航。

**暂缓方案**：本轮不新建 climb occurrence / profile cell 表，不为每个坡制造永久全局实体，不预计算全国道路双向坡库，也不把路线编辑预览写入数据库。跨路线坡搜索、同一物理坡去重和历史算法版本批量迁移只有在真实产品入口需要时才值得承担 schema、回填、去重和删除责任。

**触发条件（任一满足即重评）**：

1. 生产 `navigation_metadata_json` 的 ClimbPlan 部分 p95 超过 64KB，或任一合法路线超过 256KB；
2. 产品正式提供按 Cat/坡型/最陡 500m 跨路线筛选、附近爬坡发现或全城坡库，JSON 内嵌导致无法索引；
3. 可导出 RouteVersion 中缺少当前 ClimbPlan 的比例超过 5%，或产品决定为历史路书统一补齐爬坡计划；
4. 算法升级需要批量回填超过 100,000 个 RouteVersion，JSON 单体重写造成不可接受的锁、I/O 或回滚成本。

**检查入口**：保存/离线回填测试锁定 plan 只来自完整逐点海拔并随版本元数据落库；API 合同锁定缺失、旧版和坏 JSON 只降级为 pending；小程序静态测试锁定用户可见边界。上线后在不记录坐标和用户标识的前提下记录 `climb_plan version=<v> source=<source> point_count=<n> climb_count=<n> payload_bytes=<n> duration_ms=<ms>`；详情降级日志只记录匿名 `profile_key` 和错误类型。数据库用 JSON 字节长度分布、当前版本缺失率和 RouteVersion 数量检查容量。当前尚无生产自动告警，达到任一条件时人工重评。

**越线风险**：继续内嵌会放大 RouteVersion 元数据、无法高效做全城爬坡检索，并让批量算法升级变成大范围 JSON 重写；过早拆表则会把同一物理坡、方向、路线 occurrence、算法版本和删除级联提前固化，扩大迁移与一致性事故面。

**到点动作**：先确认触发来自 payload 体积、历史 backfill 还是跨路线检索，再分别选择压缩 profile、离线批量回填、可重建 climb projection 表或 canonical physical climb + directed occurrence 模型；迁移必须保留完整输入与旧算法结果的哈希链和 route-version 原子绑定，不能只保留最新标签。

---

## 🟡 P2 `[SCALE-TRIGGER]`：路线编辑海拔先复用按需 GLO-30 缓存，暂不建全国预热队列（2026-08-01）

**当前选择**：路线每次贴路完成后，客户端等待 800ms 的编辑静默窗口再异步请求非持久化海拔预览；下一次贴路开始时会取消尚未发出的旧预览，贴路失败或撤销当前预览后会为仍有效的确认路线重新调度。同一几何在页面内缓存，旧几何的迟到响应会被丢弃。完整路线几何只留在页面 JS 实例中用于撤销、保存和海拔，微信 `setData`/原生地图只接收最多 500 个显示点，海拔状态变化不再重传路线。腾讯单次返回的道路折线在服务端以固定 300 点预算优先保留最大转折，避免无界 RDP；只有最大横向误差不超过 1m 时才把该结果用作正式几何，否则正式几何保留完整腾讯折线，并另给最多 300 点的 `display_points` 专供地图显示，绝不使用均匀抽样改写 GPX 底片。正常正式路线仍优先在最大 1m 横向误差内压到 500 点；无法安全压缩的复杂路线允许携带最多 5000 点进入海拔与保存接口，草稿单条上限 256KB，避免“能编辑但永远保存不了”。响应距离按实际正式几何重算，腾讯原始规划距离另存为 `provider_distance_m`。异常绕行（额外超过 1km，或规划距离超过原线 1.8 倍且至少多 100m）只展示灰色预览，必须由用户选择接受或改用手绘；新客户端通过旧后端会忽略的 `X-VELO-Detour-Confirmation: 1` 请求头声明确认能力，未声明的旧客户端在新后端遇到明显绕行时直接收到 422，不能静默接受。手绘在本地直接保留触摸原线，不再调用腾讯或贴路预览接口，并抑制地图拖动/缩放结束后 450ms 内的误触加点。服务端复用正式路书的约 20m 重采样、100m 平滑和有效爬升算法，以及 API/worker 共享的 `glo30_cache` 持久卷。正式保存仍允许最长 120 秒完成冷瓦片；编辑预览单次只占用 8 秒，客户端 12 秒结束等待。未下载完的 `.part` 保留并支持下次断点续传，预览失败只显示“待重试”，不清空路线、不阻止继续编辑或保存。当前 GLO-30 不是全国离线全集，而是按 1° 瓦片首次命中后懒下载。
...
5. `requires_confirmation=1` 占有效贴路请求超过 5%，或同一路段连续被用户改用手绘，说明腾讯骑行路网不适合作为该区域的唯一贴路来源。
6. 真实保存/海拔请求 p95 超过 2000 点、请求体超过 200KB 或 API p95 增加 200ms，说明 5000 点保真兜底需要改为服务端 preview id/编码折线，而不是继续放大 JSON 合同。

**检查入口**：API 日志输出 `route_draw_elevation_preview status=<ready|unavailable|input_error> point_count=<n> duration_ms=<ms>`；贴路另输出 `route_draw_snap_preview status=<ready|unavailable|input_error> raw_point_count=<n> provider_point_count=<n> snapped_point_count=<n> display_point_count=<n> segment_count=<n> requires_confirmation=<0|1> duration_ms=<ms>`。两者都不记录坐标、路线名、用户标识或 token。生产用 `sudo docker compose logs api --since 24h | grep -E 'route_draw_(elevation|snap)_preview'` 抽样统计；GLO 持久卷容量用 `docker system df -v` 检查。前端用 121km/19 段长路线回归锁住每次地图显示不超过 500 点、海拔状态 patch 不携带路线；贴路测试锁住典型密集折线的正式几何误差不超过 1m、展示线不超过 300 点、异常绕行不自动提交、地图手势不加点。最终仍以真机连续加点无秒级冻结为准。当前没有自动告警，达到阈值后必须人工复核。
...
## 🟢 P3：约骑正式 tab 图标待替换（2026-06-01）

约骑底部 tab 暂用 leaderboard 占位图标，待 Tim 出正式 `meetup.png`+`meetup-active.png`（81×81，灰/玫红）换上。

**为什么不立即修**：需 Tim 出素材。（format 三页去重已抽 `utils/meetup-format.js` / 架构·数据流 guide 约骑章节已补，均 2026-06-02 清。）
