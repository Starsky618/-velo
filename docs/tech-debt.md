...
**当前选择**：路线每次贴路完成后，客户端等待 800ms 的编辑静默窗口再异步请求非持久化海拔预览；下一次贴路开始时会取消尚未发出的旧预览，贴路失败或撤销当前预览后会为仍有效的确认路线重新调度。同一几何在页面内缓存，旧几何的迟到响应会被丢弃。完整路线几何只留在页面 JS 实例中用于撤销、保存和海拔，微信 `setData`/原生地图只接收最多 500 个显示点，海拔状态变化不再重传路线。服务端复用正式路书的约 20m 重采样、100m 平滑和有效爬升算法，以及 API/worker 共享的 `glo30_cache` 持久卷。正式保存仍允许最长 120 秒完成冷瓦片；编辑预览单次只占用 8 秒，客户端 12 秒结束等待。未下载完的 `.part` 保留并支持下次断点续传，预览失败只显示“待重试”，不清空路线、不阻止继续编辑或保存。当前 GLO-30 不是全国离线全集，而是按 1° 瓦片首次命中后懒下载。
...
**检查入口**：API 日志输出 `route_draw_elevation_preview status=<ready|unavailable|input_error> point_count=<n> duration_ms=<ms>`；贴路另输出 `route_draw_snap_preview status=<ready|unavailable|input_error> raw_point_count=<n> snapped_point_count=<n> segment_count=<n> duration_ms=<ms>`。两者都不记录坐标、路线名、用户标识或 token。生产用 `sudo docker compose logs api --since 24h | grep -E 'route_draw_(elevation|snap)_preview'` 抽样统计；GLO 持久卷容量用 `docker system df -v` 检查。前端用 121km/19 段长路线回归锁住每次地图显示不超过 500 点、海拔状态 patch 不携带路线；最终仍以真机连续加点无秒级冻结为准。当前没有自动告警，达到阈值后必须人工复核。
