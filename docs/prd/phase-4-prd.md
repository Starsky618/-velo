...
| D22 | (subagent 多嘴提的"功率计 hardware gating"议题 / Tim 选 ii — 4.2 ship 后再单独决) | Tim 暂缓 | 2026-05-08 | 4.2 真机回归 |
| D23-D25 | （预留 / 暂未使用 / 编号占位防漂） | — | — | — |
| D26 | **power-curve 7 档**：`[0, 3, 30, 60, 300, 1200, 3600]` 替换原 6 档 / 0s 单点最大瞬时 + 3s（无氧爆发）+ 30s/1min/5min/20min（中距离）+ 1h（长距离）/ 默认 last_30_days / 前端折线图 7 个数据点连线 | Tim 拍 | 2026-05-08 | task-4.2 v2 polish |
| D27 | **heatmap markers→polylines**：后端 schema multipoint→tracks（保留 activity 边界 list of list）/ 前端 `<map polyline>` 黄色 #FFD700 + 80% alpha 重叠自然热力效果 / 视觉接近 ride.fitcard.app 80% / 删 4 张 PNG marker icon | Tim 真机回归反馈 + Claude push back | 2026-05-08 | task-4.2 v2 polish |
| D28 修订 | **未来地图 tab 用 webview + 高德 H5 SDK**（不是 Leaflet+OSM）—— OSM 不合规（测绘法 + 服务器境内要求 + 2022 屏蔽）/ Mapbox 中国版已停服 / **高德地图唯一合规且功能齐全**（dark 主题 + polyline + heatmap layer + 借测绘资质）/ Sprint 5/6 实施 | Tim 反问"开源地图能上线吗" + Claude web 查实证（OSM/Mapbox/高德对比） | 2026-05-08 | web 调研 |
| D29 | **双 component 路线**（替代单 component 万用 mode 切换）：当前 `heatmap-card`（小卡片 / 腾讯 native / 个人页）+ 未来 `heatmap-fullscreen-card`（全屏 / 高德 webview / 地图 tab）/ 二者共享 props 接口 / 内部渲染各自最优 / D21 模块化哲学的精确落实 | Tim 反问"接口能不能切换" + Claude push back | 2026-05-08 | 架构讨论 |
| D30 | **heatmap city 参数改可选**：后端 `GET /api/user/me/heatmap` city 由必填改为 Optional（`UserCity \| None = None`）/ 不传 → 显示用户全部 activity 轨迹（不论城市）/ 传 → 保留按城市筛旧行为 / 用户视角是"我所有足迹"不是"我在某城市的足迹" / 个人页 heatmap-card 砍 city props 直接拉全部 / Redis cache key 双版本互不污染（无 city: `heatmap:user_{id}` / 有 city: `heatmap:user_{id}:city_{city}`）/ Critical: 配套加 `invalidate_heatmap_cache` 工具函数 + worker activity completed hook 调用（双 key 全清）| Tim 真用反馈 + Claude push back 砍 vs 兼容 → Tim 拍兼容 | 2026-05-09 | task-4.2 v3 polish |
| D31 | **GCJ-02 坐标转换**：中国大陆 GPS 设备记录 WGS-84 国际标准 / 但腾讯地图等中国引擎用 GCJ-02 加密 / 直接传 WGS-84 偏移 100-700 米 / 必须前端转换 / 新建 `miniprogram/utils/coords.js` 含 `wgs84ToGcj02` 标准算法 / heatmap-card 内 `_convertToPolylines` 每点转 + `_computeCenter` 均值后转 / `_outOfChina` 矩形粗 guard（72-138 经度 / 0.83-55.83 纬度 / 国外坐标不转）/ 港澳台落框内会被加密（误差 < 50m 可忽略 / Tim 接受边界）| Tim 真机看到偏移 200m 反馈 + Claude 调研 GCJ-02 算法 | 2026-05-09 | task-4.2 v3 polish |
| D32 | **power-curve period 切换 UI**：power-curve-card 加 5 档 segment control（30天/90天/半年/1年/全部）/ `data.currentPeriod` 真相源 / `properties.period` observer 同步（兼容父组件强制切档）/ `_fetchAndRender` 永远读 currentPeriod / 同档点击不重 fetch 防 canvas 闪烁 / wxss iOS segment control 风格（半透明红底 + 主色字）| Tim 真机反馈"无法切换时间" / D26 隐含完整功能补 | 2026-05-09 | task-4.2 v3 polish |
